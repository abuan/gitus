...
func TaskAddProject(p *project.Project) (int, error) {
	...
	stmt, err := db.Prepare("INSERT INTO Project(name,descript,author,creation_date) VALUES(?,?,?,?)")
	if err != nil {
		return 0, err
	}
	defer stmt.Close()
	_, err = stmt.Exec(p.Name, p.Description, p.Author, p.CreationDate)
	if err != nil {
		return 0, err
	}
...
func TaskGetProject(id int) (*project.Project, []int, error) {
	...
	stmt, err := db.Prepare("SELECT name,descript,author,creation_date FROM Project WHERE id = ?")
	if err != nil {
		return nil, nil, err
	}
	defer stmt.Close()
	...
	err = row.Scan(&p.Name, &p.Description, &p.Author, &p.CreationDate)
	if err != nil {
		return nil, nil, err
	}
...
func TaskGetAllProjects() ([]*project.Project, error) {
	// Récupère les infos du projet dans la BDD
	stmt, err := db.Prepare("SELECT id,name,descript,author,creation_date FROM Project")
	if err != nil {
		return nil, err
	}
	defer stmt.Close()
	...
	for rows.Next() {
		...
		err = rows.Scan(&p.ID, &p.Name, &p.Description, &p.Author, &p.CreationDate)
		if err != nil {
			return nil, err
		}
