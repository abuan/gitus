...
func TaskAddUserStory(u *userstory.UserStory) error {
	stmt, err := db.Prepare("INSERT INTO UserStory(title,descript,effort,author,creation_date) VALUES(?,?,?,?,?)")
	if err != nil {
		return err
	}
	defer stmt.Close()
	_, err = stmt.Exec(u.Title, u.Description, u.Effort, u.Author, u.CreationDate)
	if err != nil {
		return err
	}
	return nil
...
func TaskGetUserStory(id int) (*userstory.UserStory, error) {
	...
	stmt, err := db.Prepare("SELECT title,descript,creation_date,author,effort FROM UserStory WHERE id = ?")
	if err != nil {
		return nil, err
	}
	defer stmt.Close()
	...
	err = row.Scan(&u.Title, &u.Description, &u.CreationDate, &u.Author, &u.Effort)
	if err != nil {
		return nil, err
	}
	return &u, err
...
func TaskUpdateUserStory(u *userstory.UserStory) error {
	...
	defer stmt.Close()
	_, err = stmt.Exec(u.Description, u.Effort, u.Author, u.ID)
	if err != nil {
		return err
	}
	return err
...
func TaskGetAllUserStory() ([]*userstory.UserStory, error) {
	...
	stmt, err := db.Prepare("SELECT id,title,descript,effort,author,creation_date FROM UserStory")
	if err != nil {
		return nil, err
	}
	defer stmt.Close()
	...
	for rows.Next() {
		...
		err = rows.Scan(&u.ID, &u.Title, &u.Description, &u.Effort, &u.Author, &u.CreationDate)
		if err != nil {
			return nil, err
		}
		usList = append(usList, &u)
